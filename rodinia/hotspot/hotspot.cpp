...
template<typename T>
void hotspot(T &handle, int iteration,  //number of iteration
  ...
  [[pacxx::shared]] float temp_on_cuda[BLOCK_SIZE][BLOCK_SIZE];
  [[pacxx::shared]] float power_on_cuda[BLOCK_SIZE][BLOCK_SIZE];
  [[pacxx::shared]] float temp_t[BLOCK_SIZE][BLOCK_SIZE]; // saving temporary temperature result
  ...
  int by = handle.get_block(1);

  int tx = handle.get_local(0);
  int ty = handle.get_local(1);
...
template<typename T>
int compute_tran_temp(T &MatrixPower,
                      T *MatrixTemp[2],
                      int col,
                      int row,
  ...
  int src = 0, dst = 1;
  auto dMatrixPower = MatrixPower.get();
  ...
  for (t = 0; t < total_iterations; t += num_iterations) {
    ...
    auto dMatrixTemp_src = MatrixTemp[src]->get();
    auto dMatrixTemp_dst = MatrixTemp[dst]->get();

    exec.launch([=](auto &handle) {
      hotspot(handle, iter, dMatrixPower, dMatrixTemp_src, dMatrixTemp_dst, col, row,
              borderCols, borderRows, Cap, Rx, Ry, Rz, step);
    }, {{static_cast<size_t>(blockCols), static_cast<size_t>(blockRows)}, {BLOCK_SIZE, BLOCK_SIZE}});
...
int main(int argc, char **argv) {
  ...
  auto &MatrixTemp0 = exec.allocate<float>(size);
  MatrixTemp0.upload(FilesavingTemp, size);
  auto &MatrixTemp1 = exec.allocate<float>(size);
  auto &MatrixPower = exec.allocate<float>(size);
  MatrixPower.upload(FilesavingPower, size);

  std::remove_reference_t<decltype(MatrixTemp0)> *MatrixTemp[2];
  ...
  std::vector<float> MatrixOut(size);

  MatrixTemp[ret]->download(MatrixOut.data(), MatrixOut.size());

  long long end_time = get_time();
  printf("Total time: %.3f seconds\n", ((float) (end_time - start_time)) / (1000 * 1000));

  // Write final output to output file
  writeoutput(MatrixOut.data(), grid_rows, grid_cols, ofile);
