...
double FindKeyWithDigest_CPU(const unsigned int searchDigest[4],
  ...
  double runtime = 0;//Timer::Stop(timer, "md5 runtime");
  return runtime;
}
...
// ****************************************************************************
template<typename T>
void FindKeyWithDigest_Kernel(T &handle,
  ...
  int threadid = handle.get_global(0);
...
double FindKeyWithDigest_GPU(const unsigned int searchDigest[4],
  ...
  auto &db_foundIndex = exec.allocate<int>(1);
  auto d_foundIndex = db_foundIndex.get();

  auto &db_foundKey = exec.allocate<unsigned char>(8);
  auto d_foundKey = db_foundKey.get();

  auto &db_foundDigest = exec.allocate<unsigned int>(4);
  auto d_foundDigest = db_foundDigest.get();
  //
  // initialize output buffers to show no found result
  //
  db_foundIndex.upload(foundIndex, 1);
  ...
  exec.launch([=](auto &handle) {
    FindKeyWithDigest_Kernel(handle,
                             s0,
                             s1,
                             s2,
                             s3,
  ...
  //
  db_foundIndex.download(foundIndex, 1);
  db_foundKey.download(foundKey, 8);
  db_foundDigest.download(foundDigest, 4);
